  ...
  def load(self):
    ...
    if new_key is False and os.path.exists(self.crt_file):
      ...
      issuer = subject = x509.Name([
          x509.NameAttribute(NameOID.COMMON_NAME, "Elvorath S.A.F.E Root Certificate Authority"),
          x509.NameAttribute(NameOID.ORGANIZATION_NAME, 'Elvorath'),
          x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, 'Security and Forensics Engineering'),
          x509.NameAttribute(NameOID.COUNTRY_NAME, 'US'),
          x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, 'California'),
          x509.NameAttribute(NameOID.LOCALITY_NAME, 'Placentia'),
          x509.NameAttribute(NameOID.POSTAL_CODE, '92870'),
          x509.NameAttribute(NameOID.EMAIL_ADDRESS, 'elvorath@thoughtparameters.com'),
        ])

      self.crt = x509.CertificateBuilder() \
...
class IntermediateCertificateAuthority():
  ...
  def __init__(self, key_file: str, crt_file: str, rootca_key: rsa.RSAPrivateKey, rootca_crt: x509.Certificate):
    """
  ...
  def load(self):
    ...
    if new_key is False and os.path.exists(self.crt_file):
      ...
      subject = x509.Name([
          x509.NameAttribute(NameOID.COMMON_NAME,'Elvorath S.A.F.E Intermediate Certificate Authority'),
          x509.NameAttribute(NameOID.ORGANIZATION_NAME, 'Elvorath'),
          x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, 'Security and Forensics Engineering'),
          x509.NameAttribute(NameOID.COUNTRY_NAME, 'US'),
          x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, 'California'),
      ...
      self.crt = x509.CertificateBuilder() \
        ...
        .add_extension(
          ...
          extval=x509.SubjectKeyIdentifier.from_public_key(self.key.public_key()),
            critical=False,
          ).add_extension(
          extval=x509.AuthorityKeyIdentifier.from_issuer_subject_key_identifier(
            ski=self.rootca_crt.extensions.get_extension_for_class(x509.SubjectKeyIdentifier).value
            ),
            critical=False,
          ).sign(
    ...
    if csr is None:
      ...
      cert = x509.CertificateBuilder() \
        ...
        .add_extension(
          ...
          extval=x509.SubjectAlternativeName(csr.extensions.get_extension_for_class(x509.SubjectAlternativeName).value),
          critical=False,
        ).add_extension(
          extval=x509.KeyUsage(
            digital_signature=True,
          ...
          extval=x509.ExtendedKeyUsage(usages=[
          ...
          extval=x509.AuthorityKeyIdentifier.from_issuer_subject_key_identifier(
            ski=self.crt.extensions.get_extension_for_class(x509.SubjectKeyIdentifier).value
          ),
          critical=False,
        ).sign(
          private_key=self.key,
